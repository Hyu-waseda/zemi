...
def music(image, cx, cy, landmarks, handedness):
    ...
    if handedness.classification[0].label == "Right":
        right_hand = landmark_point
    if handedness.classification[0].label == "Left":
        loudness = distance(landmark_point[8], landmark_point[4]) / distance(landmark_point[5], landmark_point[0])
        left_hand = landmark_point
        return
        

    # 音程を計算
    lst = np.linspace(15, 200, 5)
    music_lst = [psonic.A4, psonic.G4, psonic.E4, psonic.D4, psonic.C4]
    synth_lst = [psonic.FM, psonic.PIANO, psonic.PULSE, psonic.SAW, psonic.BEEP]
    if len(landmark_point) > 0:
        rate = 100 * distance(landmark_point[8], landmark_point[4]) / distance(landmark_point[5], landmark_point[0])
        print(rate)
        ...
        for i in range(len(lst)):
            if rate <= lst[i]:
                psonic.play(music_lst[i], attack=0.08, release=1.0, amp=loudness)
                break
            else: pass
            if rate >= lst[4]:
                psonic.play(music_lst[4], attack=0.08, release=1.0, amp=loudness)
        
        #音を変更
        global synth
        if len(left_hand)*len(right_hand) != 0 and right_hand[4][1]-left_hand[4][1] < 0 and right_hand[8][1]-left_hand[8][1] < 0:
            synth += 1
            psonic.use_synth(synth_lst[synth%5])
